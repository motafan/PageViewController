...
/// 自适应标记
let PageViewAutomaticDimension: CGFloat = -1

@objc public enum PageTabBarItemTransitionAnimation: Int {
    /// 无动画
    case none
    /// 平滑的
    case smoothness
...
// MARK: -  行为代理
@objc public protocol PageTabBarDelegate: AnyObject {
    ...
    ///   - index: 当前index
    @objc optional func pageTabBar(_ pageTabBar: PageTabBar, didSelectedItemAt index: Int)
    ...
    ///   - toIndex: toIndex
    ///   - percent: 进度
    @objc optional func pageTabBar(_ pageTabBar: PageTabBar, didScrollItem fromIndex: Int, toIndex: Int, percent: CGFloat)
    
}

// MARK: -  数据源代理
@objc public protocol PageTabBarDataSource: AnyObject {
    ...
    /// - Returns: 默认选中位置
    @objc optional func defaultSelectedIndex(in pageTabBar: PageTabBar) -> Int
    ...
    @objc optional func pageTabBar(_ pageTabBar: PageTabBar, widthForItemAt index: Int) -> CGFloat
    
    /// 默认字体，默认为15号系统字体
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: 默认字体
    @objc optional func titleFontForItem(in pageTabBar: PageTabBar) -> UIFont
    
    /// 高亮字体，默认为15号系统字体
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: 高亮字体
    @objc optional func titleHighlightedFontForItem(in pageTabBar: PageTabBar) -> UIFont
    
    /// 默认字体颜色，默认为浅灰色
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: 默认字体颜色
    @objc optional func titleColorForItem(in pageTabBar: PageTabBar) -> UIColor
    
    /// 高亮字体颜色，默认为黑色
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: 高亮字体颜色
    @objc optional func titleHighlightedColorForItem(in pageTabBar: PageTabBar) -> UIColor

    /// item之间的间距，默认为10
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: item之间的间距
    @objc optional func spacingForItem(in pageTabBar: PageTabBar) -> CGFloat
    
    /// 是否开启当item总宽度小于总的宽度时居中显示所有item，并重新计算item之间的间距，默认开启
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: 是否开启
    @objc optional func relayoutWhenWidthNotEnough(in pageTabBar: PageTabBar) -> Bool
    
    /// 是否需要指示器，默认为true
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: 是否需要指示器
    @objc optional func needsIndicatorView(in pageTabBar: PageTabBar) -> Bool

    /// 指示器的颜色，默认为选中字体颜色
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: 指示器的颜色
    @objc optional func colorForIndicatorView(in pageTabBar: PageTabBar) -> UIColor
    
    /// 指示器的高度，默认为3
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: 指示器的高度
    @objc optional func heightForIndicatorView(in pageTabBar: PageTabBar) -> CGFloat
    
    /// 指示器距离底部的位置，默认为5
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: 指示器距离底部的位置
    @objc optional func bottomForIndicatorView(in pageTabBar: PageTabBar) -> CGFloat
    ...
    ///   - index: 位置
    /// - Returns: 宽度
    @objc optional func pageTabBar(_ pageTabBar: PageTabBar, widthForIndicatorViewAt index: Int) -> CGFloat
    
    /// 切换动画，默认为none
    ///
    /// - Parameter pageTabBar: pageTabBar
    /// - Returns: 切换动画
    @objc optional func transitionAnimationType(in pageTabBar: PageTabBar) -> PageTabBarItemTransitionAnimation
...
open class PageTabBar: UIView {
    ...
    /// 回调代理
    open weak var delegate: PageTabBarDelegate?
    
    /// 数据源代理
    open weak var dataSource: PageTabBarDataSource? {
        didSet {
    ...
    /// 数据模型
    private var itemModels: [PageTabBarItemModel] = []
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(PageTabBarItem.self, forCellWithReuseIdentifier: PageTabBarItem.reuseID)
...
extension PageTabBar {
    ...
    private func widthForIndicatorView(at index: Int) -> CGFloat {
        ...
        if let width = dataSource?.pageTabBar?(self, widthForIndicatorViewAt: index) {
            if width != PageViewAutomaticDimension {
                return width
            }
        }
    ...
    private func widthForItem(at index: Int, font: UIFont? = nil) -> CGFloat {
        ...
        /// 计算宽度
        if let font = font {
            return PageTabBarUtil.stringWidth(with: title, font: font)
        }
        ...
        return PageTabBarUtil.stringWidth(with: title, font: titleFont)
    }
    ...
    ///
    /// - Returns: 切换动画
    private func transitionAnimationType() -> PageTabBarItemTransitionAnimation {
...
extension PageTabBar {
    ...
    private func refreshCurrentState() {
        ...
        /// 计算当item总宽度小于PageTabBar的宽度时，居中显示所有item，并重新计算itemSpacing，
    ...
    ///   - percent: 滑动进度，默认为1
    /// - Returns: model
    private func itemModel(at index: Int, percent: CGFloat = 1) -> PageTabBarItemModel {
        ...
        if index != selectedIndex {
            isSelected = false
            titleFont = PageTabBarUtil.interpolationFont(fromFont: titleHighlightedFontForItem(), toFont: titleFontForItem(), percent: percent)
            titleHighlightedFont = titleHighlightedFontForItem()
            titleColor = PageTabBarUtil.interpolationColor(fromColor: titleHighlightedColorForItem(), toColor: titleColorForItem(), percent: percent)
            titleHighlightedColor = titleHighlightedColorForItem()
            itemWidth = widthForItem(at: index, font: titleFont)
        } else {
            isSelected = true
            titleHighlightedFont = PageTabBarUtil.interpolationFont(fromFont: titleFontForItem(), toFont: titleHighlightedFontForItem(), percent: percent)
            titleFont = titleFontForItem()
            titleHighlightedColor = PageTabBarUtil.interpolationColor(fromColor: titleColorForItem(), toColor: titleHighlightedColorForItem(), percent: percent)
            titleColor = titleColorForItem()
            itemWidth = widthForItem(at: index, font: titleHighlightedFont)
        }
        let itemModel = PageTabBarItemModel(title: title, titleFont: titleFont, titleHighlightedFont: titleHighlightedFont, titleColor: titleColor, titleHighlightedColor: titleHighlightedColor, itemWidth: itemWidth, isSelected: isSelected)
        return itemModel
    }
    ...
    private func refreshItemState(fromIndex: Int, toIndex: Int, percent: CGFloat) {
        guard let fromItem = collectionView.cellForItem(at: IndexPath(item: fromIndex, section: 0)) as? PageTabBarItem,
            let toItem = collectionView.cellForItem(at: IndexPath(item: toIndex, section: 0)) as? PageTabBarItem else {
                return
        }
        ...
        if let indicatorView = indicatorView {
            ...
            let centerX = PageTabBarUtil.interpolationValue(fromValue: fromItemFrame.midX, toValue: toItemFrame.midX, percent: percent)
            ...
            let indicatorWidth = PageTabBarUtil.interpolationValue(fromValue: fromWidth, toValue: toWidth, percent: percent)
...
extension PageTabBar: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: PageTabBarItem.reuseID, for: indexPath) as! PageTabBarItem
